...
*   **天氣（啟用「顯示天氣」時）**：使用免費的 Open-Meteo 服務，**完全不需要瀏覽器定位權限**。本地天氣會將您**手動輸入的城市名稱**送至 Open-Meteo Geocoding API (`https://geocoding-api.open-meteo.com/`) 解析為座標；世界時鐘（第二／第三時鐘）的天氣則使用該城市內建或自訂解析的座標。接著以座標向 Open-Meteo 預報 API (`https://api.open-meteo.com/`) 取得目前氣溫與天氣狀態。查詢結果會在本機快取約 30 分鐘以減少請求。
...
*   **Weather (when "Show weather" is enabled)**: Uses the free Open-Meteo service and requires **no browser geolocation permission**. Local weather sends the **city name you type in** to the Open-Meteo Geocoding API (`https://geocoding-api.open-meteo.com/`) to resolve coordinates; world-clock (second/third clock) weather uses that city's built-in or resolved coordinates. The coordinates are then sent to the Open-Meteo Forecast API (`https://api.open-meteo.com/`) to get the current temperature and condition. Results are cached locally for about 30 minutes to reduce requests.
