...
const AMO_ADDON_ID = 'just-new-tab@ivanusto.yblog.org';
...
function createZipForPlatform(platform, version) {
  const srcDirName = platform === 'chrome' ? 'just-new-tab-chrome' : 'just-new-tab';
  const distPlatformDir = path.join(__dirname, srcDirName);
  const zipPath = path.join(__dirname, 'dist', `just-new-tab-${platform}-v${version}.zip`);
  ...
  if (process.platform === 'win32') {
    ...
    execSync(`cd "${distPlatformDir}" && zip -r -X "${zipPath}" . -x "*.DS_Store" "*.md"`);
  }
  return zipPath;
}
...
(async () => {
  ...
  const version = JSON.parse(fs.readFileSync(path.join(__dirname, 'just-new-tab', 'manifest.json'), 'utf8')).version;
